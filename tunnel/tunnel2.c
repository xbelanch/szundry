...
void createLookUpTable(Table *table, int widthScreen, int heightScreen)
{
    ...
    float big_number = (60.0 * TEXTURE_HEIGHT);
    for (int row = 0; row < heightScreen; ++row) {
        for (int col = 0; col < widthScreen; ++col) {
            ...
            table->distance[row * widthScreen + col] = ((int)(big_number / inverse_distance)) % TEXTURE_HEIGHT;
            table->angle[row * widthScreen + col] = (int)(0.5 * TEXTURE_WIDTH *
