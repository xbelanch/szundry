...
static void check_video_display_before_start_the_game()
{

	LOG("\n"
		"Check video display avalaibles:\n"
		"===============================\n");
	int n = SDL_GetNumVideoDisplays();
	if (n > 1)
		LOG("Number of video displays = %d\n", n);

	n = SDL_GetNumVideoDrivers();
	LOG("Video drivers (%i):", n);

	for (int i = 0; i < n; i++)
		LOG(" '%s'", SDL_GetVideoDriver(i));
